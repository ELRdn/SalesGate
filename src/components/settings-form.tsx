  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setValues({ ...values, slack_webhook_url: e.target.value })}
              className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm font-mono focus:border-emerald-500 focus:outline-none"
            />
            <p className="mt-1 text-xs text-zinc-500">
              承認待ちが発生したときにSlackへ通知します。空の場合は通知無効。
            </p>
          </div>
          <button
            disabled={isPending}
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          <h2 className="mb-2 font-semibold">📤 送信履歴のエクスポート</h2>
          <p className="mb-3 text-sm text-zinc-500">
            送信履歴（MessageLog）をCSVでダウンロードします（Excel対応・最新5000件）。
          </p>
          <a
            href="/api/export/logs"
            className="block w-full rounded-lg bg-zinc-700 px-4 py-2 text-center text-sm font-medium hover:bg-zinc-600"
          >
            ⬇️ CSVをダウンロード
          </a>
        </div>
